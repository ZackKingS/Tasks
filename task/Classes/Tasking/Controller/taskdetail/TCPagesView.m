...
- (void)setUpView
{
    ...
    frame.size.height = _headHeight + 50+100 -50  -100;
    frame.origin.y = -100 +  100 ;
                       ...
                       context:(void *)context
{
    ...
    if (context == &TcPagingViewScrollContext)
    {
        ...
        if (-newOffsetY < headerViewHeight  && -newOffsetY < _segmentViewHeight)
        {
            //上面的划出屏幕 上面的固定
        ...
        else if(newOffsetY<- _headHeight - _segmentViewHeight)
        {
            ...
            CGRect frame = self.headerView.frame;
            frame.origin.y = -100.f + offset * 0.6 + 100.f;
            CGFloat height = _headHeight + 100.f + 50.f + offset *0.4  -50 -100.f;
